...
def exist_arc(node1, node2, Arc_list):
    '''判断边是否存在'''
    for each in Arc_list:
        if node1.info in each.node_info_list and node2.info in each.node_info_list:
            return True
    return False

def get_degree(VNode, Arc_list):
    '''给定一个结点,和边列表求该点的度数'''
    degree = 0
    for each_arc in Arc_list:
        if VNode.info in each_arc.node_info_list:
            degree += 1
    return  degree
...
if __name__ == '__main__':
    ...
    # 求度数
    for each_node in Node_list:
        print(each_node.info, "度数为:", get_degree(each_node, Arc_list))

    # 判断边是否存在
    if exist_arc(Node_list[0],Node_list[2],Arc_list):
        print("存在")
    else:
        print("不存在")

    if exist_arc(Node_list[0],Node_list[3],Arc_list):
        print("存在")
    else:
        print("不存在")
